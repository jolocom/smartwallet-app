...
export const scheduleNotification = (
  ...
  dispatch(scheduleToast(notification))
 ...
 */
export const invokeDismiss = (notification: Notification): ThunkAction => (
  dispatch,
) => {
...
const getActiveNotificationState = (getState: () => RootReducerI) => {
  const curTs = Date.now()
  const { active, activeExpiryTs } = getState().toasts
...
const updateNotificationsState: ThunkAction = async (dispatch, getState) => {
  ...
  if (next && next !== active) {
    ...
    ret = dispatch(setActiveToast({ notification: next, expiry: nextExpiry }))
  }
...
const clearActiveNotification = () =>
  setActiveToast({ notification: null, expiry: 0 })
