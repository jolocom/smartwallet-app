...
export const useMagicBtnAnimations = (animationDuration: number) => {
  ...
  // on Android.
  const magicOpacity = useRef<Animated.Value>(new Animated.Value(INITIAL_MAGIC_BTN_OPACITY)).current
  const shadowScale = useRef<Animated.Value>(new Animated.Value(INITIAL_SHADOW)).current
  const circleScale = useRef<Animated.Value>(new Animated.Value(INITIAL_CIRCLE_SCALE)).current

  const animate = (value: Animated.Value | Animated.ValueXY, toValue: number, duration: number) => Animated.timing(value, {
    toValue,
    duration: duration,
