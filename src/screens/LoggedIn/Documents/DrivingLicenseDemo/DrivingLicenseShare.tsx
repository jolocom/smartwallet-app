    ...
    return (
        <ScreenContainer customStyles={{justifyContent: 'flex-start'}}>
            <NavigationHeader type={NavHeaderType.Close}>
                <JoloText color={Colors.white}>Share Driving License</JoloText>
            </NavigationHeader>
            <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
                <View style={{borderWidth: 20, borderColor: Colors.white, borderRadius: 14}}>
                    {qrCodeString ? (
                            <QRCode value={qrCodeString} />
                    ) : (
                        <ActivityIndicator size={"large"} style={{padding: 20, backgroundColor: Colors.white}} />
                    )}
                </View>
            </View>
        </ScreenContainer>
