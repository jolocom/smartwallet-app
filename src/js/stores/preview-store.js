  ...
  onNavigateToNode: function(node) {
    ...
    this.gAgent.getGraphMapAtUri(node.uri).then((triples) => {
      ...
      if (this.state.navHistory.length > 1) {
        let hist = this.state.navHistory[this.state.navHistory.length - 2].uri
        if (this.state.center.uri === hist) {
          ...
          for (var j = 0; j < this.state.navHistory.length - 1; j++) {
            ...
            let hist = this.state.navHistory[index].uri
            if (this.state.center.uri === hist) {
              for (var k = 0; k < j + 2; k++) {
                this.state.navHistory.pop()
              }
      ...
      for (var i = 1; i < triples.length; i++) {
        triples[i] = this
          .convertor
          .convertToD3('a', triples[i], i, triples.length - 1)
