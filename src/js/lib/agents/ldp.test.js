...
describe('LDPAgent', function() {
  describe('#createBasicContainer', function() {
    it('should be create a basic container (directory)', async function() {
      ...
      agent.normalHTTP._fetch = async (url, options) => {
          ...
          expect(options.headers['Link']).to.equal('<http://www.w3.org/ns/ldp#BasicContainer>; rel="type"')
  ...
  describe('#findTriples', function() {
    it('it should be able to fetch and filter triples located at a URI', async function() {
      const agent = new LDPAgent
      agent.proxiedHTTP._fetch = async (url, options) => {
          expect(options.method).to.equal('GET')
          expect(url).to.equal('http://foo.com/test')
          return {
              status: 200, ok: true, headers: {get: (field) => ({'Content-Type': 'text/turtle'})[field]},
              text: async () => `
...
`.trim()
      ...
      expect(result[0].predicate.value).to.equal('http://xmlns.com/foaf/0.1/knows')
  ...
  describe('#fetchTriplesAtUri', function() {
    it('it should be able to fetch triples located at a URI', async function() {
      const agent = new LDPAgent
      agent.proxiedHTTP._fetch = async (url, options) => {
          expect(options.method).to.equal('GET')
          expect(url).to.equal('http://foo.com/test')
...
`.trim()
      ...
      const result = await agent.findTriples('http://foo.com/test')
      expect(result[1].predicate.value).to.equal('http://xmlns.com/foaf/0.1/knows')
    })
  })
  
  describe('#getAclUri', function() {
    it('should be able to extract ACL location from Link header', async function() {
      const agent = new LDPAgent
      agent.proxiedHTTP._fetch = async (url, options) => {
          expect(options.method).to.equal('HEAD')
          expect(url).to.equal('http://foo.com/test')
          return {
              status: 200, ok: true, headers: {get: (field) => ({'Content-Type': 'text/rdf',
                                                                 'Link': '<testAcl>; rel="acl"'})[field]},
              text: async () => ''
          }
      }
    ...
    it('should guess in the case a Link header is not present', async () => {
      ...
      agent.proxiedHTTP._fetch = async (url, options) => {
          ...
          expect(url).to.equal('http://foo.com/test')
          return {
              status: 200, ok: true, headers: {get: (field) => ({'Content-Type': 'text/rdf'})[field]},
              text: async () => ''
          }
      }
    ...
    it('should guess in the case a rel=acl header is not present even if there are other Link headers', async () => {
      const agent = new LDPAgent
      agent.proxiedHTTP._fetch = async (url, options) => {
          expect(options.method).to.equal('HEAD')
          expect(url).to.equal('http://foo.com/test')
