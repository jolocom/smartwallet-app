/**
 * TouchRotate
 * @param DOMElement touchElement The element whose center we use for the
 * rotation, and on which the touch events occur; musn't be an svg element
...
var TouchRotate = function (touchElement, callbacks) {
  ...
  // Get radian starting from standard CSS transform axis
  // (vertical top axis: ((0,0),(0,1)))
  function getRadian(currentX, currentY, centerX, centerY) {
    var opp = centerY - currentY
    var adj = currentX - centerX
    var radStartingRight = Math.atan2(opp, adj)
    return Math.PI / 2 - radStartingRight
  }

  // Handle mobile and desktop mouse events for rotation
  ['touchstart', 'mousedown', 'touchmove', 'mousedownmove']
    .forEach(function (eventName) {
      touchElement.addEventListener(eventName, function (e) {
        var currentX = e.touches ? e.touches[0].pageX
        : (e.pageX ? e.pageX : e.detail.pageX)
        var currentY = e.touches ? e.touches[0].pageY
        : (e.pageY ? e.pageY : e.detail.pageY)
