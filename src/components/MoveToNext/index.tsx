...
const MoveToNextContext = React.createContext<IMoveToNextContext>({
  inputs: { current: [] },
})
...
const MoveToNext: React.FC & IMoveToNextComposition = ({ children }) => {
  ...
  return <MoveToNextContext.Provider value={{ inputs }} children={children} />
}

const InputsCollector: IMoveToNextComposition['InputsCollector'] = ({
  children,
  onSubmit,
}) => {
