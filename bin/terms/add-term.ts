...
const printHelp = () => {
  ...
  console.log(
    '\x1b[0;95m%s\x1b[0m',
    'yarn term:add --ctx=Term --term=testTerm --content=testing --lang=de',
  )
  console.log('')
  ...
  console.log(
    "--lang          (optional) 'en' | 'de' - term translation language, defaults to 'en'",
  )
}
...
const addTranslation = async (mode: 'add' | 'update' = 'add') => {
  if (lang !== Languages && mode === 'add') {
    ...
    console.log(
      '\x1b[0;93m%s\x1b[0m',
      `Provided lang argument "${lang}" is not supported.`,
    )
  ...
  if (
    data &&
    data.response.code === '200' &&
    data.result.translations[key] === 1
  ) {
    return data
  } else if (
    data &&
    data.response.code === '200' &&
    data.result.translations[key] === 0
  ) {
    if (mode === 'add') {
      const { update: shouldUpdate } = await inquirer.prompt(
        questionUpdateTranslation,
      )
      if (shouldUpdate) {
...
if (!ctx || !term || !content) {
  ...
  const updatedQuestions = questionsAddingTerm.filter(
    (q) => !Boolean(args[q.name]),
  )
  ;(async () => {
